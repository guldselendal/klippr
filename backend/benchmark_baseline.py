#!/usr/bin/env python3
"""
Baseline performance measurement script for database save operations.
Run this before optimizations to establish baseline metrics.
"""
import time
import uuid
...
def create_test_data(num_chapters=100, chapter_size=5000):
    ...
    for i in range(num_chapters):
        chapters_data.append({
            ...
            'document_id': file_id,
            'title': f'Chapter {i+1}',
            'content': 'X' * chapter_size,  # Simulate chapter content
            'summary': 'Summary text ' * 50,  # ~500 chars
            'preview': 'Preview text ' * 20,  # ~200 chars
            'chapter_number': i + 1
        })
    
    return file_id, chapters_data

def baseline_save_method(db, file_id, chapters_data, display_title="Test Document"):
    """Current save method - per-row adds"""
    ...
    # Add chapters one by one (current method)
...
def measure_baseline(num_chapters=100, chapter_size=5000, iterations=3):
    ...
    for iteration in range(iterations):
        ...
        try:
            ...
            # Measure save
            start = time.time()
            baseline_save_method(db, file_id, chapters_data)
            elapsed = time.time() - start
...
if __name__ == "__main__":
    ...
    measure_baseline(num_chapters=10, chapter_size=2000, iterations=3)
    
    # Medium: 50 chapters
    print("\n📊 Medium document (50 chapters)")
    measure_baseline(num_chapters=50, chapter_size=5000, iterations=3)
    
    # Large: 100 chapters
    print("\n📊 Large document (100 chapters)")
    measure_baseline(num_chapters=100, chapter_size=5000, iterations=3)
